!!**********************************************************************
!!*   Computes phonon transport from a general Hessian                 *
!!*
!!* Developers: A. Pecchia, L. Medrano                                 *
!!*
!!**********************************************************************
program phonons 
  use globalenv
  ...
  if (tTransport) then
    writeTunn = .true.
    writeLDOS = .true.
    call negf_init(transpar, tundos, env%mpi%globalComm, atTemperature, tInitialized)
    ...
    call calc_phonon_current(env%mpi%globalComm, dynMatrix, tunnTot, ldosTot, currTot, &
                        & writeTunn, writeLDOS)  
